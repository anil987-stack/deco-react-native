...
import JodidarInfo from "./JodidarInfo";
import JodidarQues from "./JodidarQues";
import WhiteButton from "App/Components/WhiteButton";
import Communication from "./Communication";
import Financial from "./Financial";
import BusinessType from "./BusinessType";

class Jodidar extends Component {
  submit(params) {
  ...
  onCheck() {
      ...
      records: [
        {
          ...
          AE_items1__c: form.AE_items1__c,
          Value_AE_items1__c: form.Value_AE_items1__c,
          AE_items2__c: form.AE_items2__c,
          ...
          Suggestions_on_New_Product_launch__c: form.Suggestions_on_New_Product_launch__c
            ...
            : "",
          Overall_Feedback_about_LIS_LAD_concept__c:
          ...
          Brand_Lube__c: form.Brand_Lube__c,
          Value_Lube__c: form.Value_Lube__c,
    ...
    if (!form.Name) {
      HelperService.showToast({
        message: "Jodidars Name Field is empty",
      });
    } else if (!form.Phone) {
      HelperService.showToast({
        message: "Contact Person Mob No Field is empty",
      });
    } else if (!this.validatePhoneNumber(form.Phone)) {
      HelperService.showToast({
        message: "Contact Person Mob No is not Valid",
      });
    } else if (!this.validatePhoneNumber(form.Alternate_WhatsApp_No__c)) {
      HelperService.showToast({
        message: "Alternate Mob No is not Valid",
      });
    ...
    } else if (!form.Mechanic_Type__c) {
      HelperService.showToast({
        message: "Select Mechanic Type",
      });
    } else if (!form.ParentId) {
      HelperService.showToast({
        message: "Select Authorized LAD/Dealer/Retailer name attached with",
      });
    ...
    } else if (!form.Monthly_Offtake_Fitment_Value_in_Lacs__c) {
      HelperService.showToast({
        message: "Monthly Offtake/Fitment (Value in Lacs) is empty",
      });
    ...
    } else if (!form.AE_items1__c) {
      HelperService.showToast({
        message:
          "Select Brand for AE items (Self & Alternator & its Child parts)",
      });
    } else if (!form.Value_AE_items1__c) {
      HelperService.showToast({
        message:
          "Enter Value for AE items (Self & Alternator & its Child parts)",
      });
    } else if (!form.AE_items2__c) {
      HelperService.showToast({
        message:
          "Select Brand for AE items (Horns, Cable, Switches, Bulbs, Blades, Glow Plugs,etc)",
      });
    } else if (!form.Value_AE_items2__c) {
      HelperService.showToast({
        message:
          "Enter Value for AE items (Horns, Cable, Switches, Bulbs, Blades, Glow Plugs,etc)",
      });
    } else if (!form.Brand_Battery__c) {
      HelperService.showToast({
        message: "Select Brand For Battery",
      });
    } else if (!form.Value_Battery__c) {
      HelperService.showToast({
        message: "Enter Value for Battery",
      });
    } else if (!form.Brand_Lube__c) {
      HelperService.showToast({
        message: "Select Brand For lube",
      });
    } else if (!form.Value_Lube__c) {
      HelperService.showToast({
        message: "Enter Value for lube",
      });
    } else if (!form.Brand_Filters__c) {
      HelperService.showToast({
        message: "Select Brand for Filters",
      });
    } else if (!form.Value_filters__c) {
      HelperService.showToast({
        message: "Enter Value for Filters",
      });
    } else if (!form.Brand_2_3_Wheel_ranges__c) {
      HelperService.showToast({
        message: "Select Brand for 2-3 Wheels ranges",
      });
    } else if (!form.Value_2_3_Wheel_ranges__c) {
      HelperService.showToast({
        message: "Enter Value for 2-3 Wheels ranges",
      });
    } else if (!form.Any_Quality_issue_with_product_Part_No__c) {
      HelperService.showToast({
        message: "Quality Issue field is empty",
      });
    } else if (!form.Weekly_Holiday__c) {
      HelperService.showToast({
        message: "Weekly Holiday field is empty",
      });
    } else if (!form.Have_you_got_all_you_settlement__c) {
      HelperService.showToast({
        message: "Select Have you received all your settlement",
      });
    } else if (!form.Overall_Feedback_about_LIS_LAD_concept__c) {
      HelperService.showToast({
        message: "Overall Feedback about LIS & LAD concept is Empty",
      });
    } else {
      ...
      batchRequests: [
        {
          ...
          richInput: {
            ...
            Brand_Lube__c: form.Brand_Lube__c ? form.Brand_Lube__c : "",
            Value_Lube__c: form.Value_Lube__c ? form.Value_Lube__c : "",
        ...
        <Content  theme={{backgroundColor:"red"}} > */}
          ...
          >
            <Tab
              ...
              tabStyle={{ backgroundColor: Colors.primary, paddingHorizontal: 0, }}
              activeTextStyle={{ color: "#fff", fontSize: 15 }}
              activeTabStyle={{ backgroundColor: Colors.primary }}
            >
              <JodidarInfo Id={id.show} />
            </Tab>
            <Tab
              heading="GENERAL QUESTIONS & FEEDBACK"
...
export default connect(mapStateToProps, mapDispatchToProps)(Jodidar);
