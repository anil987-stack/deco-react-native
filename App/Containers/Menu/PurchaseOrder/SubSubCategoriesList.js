...
export default class SubSubCategoriesList extends React.Component {
	getCardNode(data) {
		...
		if (data.header == true) {
			...
			if (selectedData.indexOf(data.Application__c) > -1) {
				return (
		            <ListItem selected={true} onPress={() => onDeselect(data.Application__c)}>
			            <CheckBox checked={true} color={Colors.button}  onPress={() => onDeselect(data.Application__c)}/>
				            <Body>
				...
				return (
					<ListItem selected={false} onPress={() => onSelect(data.Application__c)}>
			            <CheckBox checked={false}  color={Colors.button} onPress={() => onSelect(data.Application__c)}/>
				            <Body>
