	...
	getCardNode(data) {
		...
		if (data.header == true) {
			...
			if (selectedData.indexOf(data.Name) > -1) {
				return (
		            <ListItem selected={true} onPress={() => onDeselect(data.Name)}>
			            <CheckBox checked={true} color={Colors.button}  onPress={() => onDeselect(data.Name)}/>
				            <Body>
				...
				return (
					<ListItem selected={false} onPress={() => onSelect(data.Name)}>
			            <CheckBox checked={false}  color={Colors.button} onPress={() => onSelect(data.Name)}/>
				            <Body>
