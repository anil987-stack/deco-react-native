...
export default class SubCategoriesList extends React.Component {
  getCardNode(data) {
    ...
    if (data.header == true) {
      ...
      if (selectedData.indexOf(data.Product_Type_2__c) > -1) {
        return (
          <ListItem selected={true} onPress={() => onDeselect(data.Product_Type_2__c)}>
            <CheckBox
              checked={true}
              color={Colors.button}
              onPress={() => onDeselect(data.Product_Type_2__c)}
            />
            <Body>
              <Text style={Style.selectedListItemText}>
                {data.Product_Type_2__c}
              </Text>
            </Body>
          </ListItem>
        );
        ...
        return (
          <ListItem selected={false} onPress={() => onSelect(data.Product_Type_2__c)}>
            <CheckBox
              checked={false}
              color={Colors.button}
              onPress={() => onSelect(data.Product_Type_2__c)}
            />
            <Body>
              <Text style={Style.listItemText}>{data.Product_Type_2__c}</Text>
            </Body>
          </ListItem>
        );
      }
