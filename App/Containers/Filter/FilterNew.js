...
class Filter extends Component {
  ...
  filterResults(list) {
    ...
    let filteredList = HelperService.searchTextListFilter(
      list,
      "name",
      searchFilter["searchByName"]
    );
    filteredList = HelperService.searchTextListFilter(
      filteredList,
      "parent",
      searchFilter["parentName"]
    );
    ...
    return dataList.map((item, key) => {
      return (
        <TouchableOpacity
          ...
          onPress={() => {
            if (
              HelperService.searchInList(
                searchFilter &&
                  searchFilter[item.parent] &&
                  searchFilter[item.parent].length
                  ? searchFilter[item.parent]
                  : [],
                item.parent == "Geographical_Town" || item.parent == "Territory"
                  ? item.id
                  : item.name
              )
            ) {
              this.removeSelectedOption({
                parent: item.parent,
                item:
                  item.parent == "Geographical_Town" ||
                  item.parent == "Territory"
                    ? item.id
                    : item.name,
              });
            } else {
  ...
  addSelectedOption(params) {
    ...
    if (
      searchFilter &&
      searchFilter[parentLabel] &&
      searchFilter[parentLabel].length &&
      parentLabel != "Team_PJP"
    ) {
  ...
  render() {
    const { listMain, changeFilters, pjpFilters, searchFilter } = this.props;

    let parentList =
      listMain && listMain.length
        ? HelperService.getParentfromList(listMain)
        : [];
    // let parentList = listMain
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        >
          <View style={{ width: wp("36%"), marginTop: hp("1") }}>
            {parentList.map((item, index) => {
              return (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    changeFilters({
                      edited_field: "parentName",
                      edited_value: item.parent,
                    })
                  }
                  style={[
                    styles.menuItem,
                ...
                >
                  ...
                  >
                    {item.parent ? item.parent.replace(/_/g, " ") : ""}
                  </Text>
                </TouchableOpacity>
              );
            })}
          ...
          <View>
            ...
            <ScrollView style={{ left: "5%", height: wp("105%") }}>
        ...
        >
          ...
          {/* <BlueButton
            style={{ ...styles.button, right: wp("0"), marginLeft: wp("25") }}
            title={"Apply"}
            onPress={() => {
              this.onPressApply();
            }}
...
const mapDispatchToProps = (dispatch) => ({
  changeFilters: (params) =>
    dispatch(CommonActions.changeCommonFilters(params)),
