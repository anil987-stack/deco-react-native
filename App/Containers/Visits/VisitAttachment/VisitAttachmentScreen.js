...
import Style from "./VisitAttachmentScreenStyle";
...
import ImagePick from "../../../Components/ImagePicker/ImagePick";
import SearchableDropdown from "App/Components/SearchableDropdown";

let extractData = {};
class VisitAttachmentScreen extends Component {
  componentDidMount() {
    this.fetchcall();
  }
  fetchcall() {
    const { fetchLocalImage, token, item, executeVisitData } = this.props;
    fetchLocalImage({
      token,
  ...
  constructor(props) {
    ...
    // console.log(this.state.fileUri, "ghdghfggf");
  }
  ...
  render() {
    ...
    let id = executeVisitData.Id;
    console.log("selectLocalExpense", type);

    let visibleNode = (
      <ScrollView style={Style.action}>
        <FilePicker
          onSuccess={({ fileName, fileUri }) => {
            this.setState({ fileName: fileName, fileUri: fileUri });
            console.log(this.state.fileUri, "hhhhhhhhh");
          }}
        />
        {/* <View style={{alignSelf:'center'}}>
<Text style={{color:'red',fontWeight:'bold',fontSize:20,marginTop:'20%'}}>Can't add more Images or Files </Text>
</View> */}
    ...
    return (
      <View style={Style.container}>
        ...
        {/* <Text style={{ fontSize: 20 }}>
          {imagelist && imagelist[id] ? imagelist[id].length + "/4" : ""}
        </Text> */}
...
const mapDispatchToProps = (dispatch) => ({
  clearSelectLocalItemExpense: (params) =>
    dispatch(LocalActions.clearSelectLocalItemExpense(params)),
