...
class Tickets extends Component {
  componentDidMount() {
    const { getDealerPoints, token, selectedRetailer } = this.props;
  ...
  getCardNode(data) {
    return <PointCard data={data} id={data.Id} />;
  }
...
const mapStateToProps = (state) => ({
  data: state.retailers.dealerPoints,
  token: state.user.token,
  loading: state.retailers.dealerPointsLoader,
  selectedRetailer: state.retailers.selectedRetailer,
});

const mapDispatchToProps = (dispatch) => ({
  getDealerPoints: (params) =>
    dispatch(RetailerActions.getDealerPoints(params)),
});
